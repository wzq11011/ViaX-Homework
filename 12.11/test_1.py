...
Portrait_noback_data = DataLoader(Portrait_noback_set, batch_size = 16, shuffle = True, drop_last = True)

Back_set = ImageFolder('./landmark1_big', transform = tfs_background)
Back_data = DataLoader(Back_set, batch_size = 16, shuffle = True, drop_last = True)

Portrait_withback_set = ImageFolder('./portrait_with_back', transform = tfs_portrait)
Portrait_withback_data = DataLoader(Portrait_withback_set, batch_size = 16, shuffle = True, drop_last = True)
...
FCN_optim = optim.SGD(back_FCN.parameters(), lr = 0.001)
Discriminator_optim = optim.SGD(Discriminator.parameters(), lr = 0.001)
...
for epo in range(epoch):
    ...
    for back, P_noback, P_withback in zip(Back_data, Portrait_noback_data, Portrait_withback_data):
        ...
        composit_portrait = back_FCN(back[0].to(device)) + P_noback[0].to(device)
